...
This plugin is an addon for the popular [WPGraphQL](https://github.com/wp-graphql/wp-graphql) plugin. It helps with setting up GraphCDN in front of a WordPress GraphQL API by automatically taking care of invalidating the cache when content is updated in WordPress. If works no matter the source of the update (WordPress admin panel, GraphQL mutation, REST API, etc.). It also supports custom post types and custom taxonomies that are exposed over the GraphQL API.
